-- dau, iap, ad new 
CREATE OR REPLACE TABLE `bounce-survivor---puzzle-rpg.dashboard_table.dau_new_iap`
...
CREATE OR REPLACE TABLE `bounce-survivor---puzzle-rpg.dashboard_table.retention`
PARTITION BY event_date
CLUSTER BY version, platform, country AS
with user_d0 as 
(
  select distinct 
...
from d0 left join d1 on d0.event_date = d1.event_date and d0.platform = d1.platform and d0.country = d1.country and d0.version = d1.version
...
-- playtime
CREATE OR REPLACE TABLE `bounce-survivor---puzzle-rpg.dashboard_table.playtime`
PARTITION BY event_date
...
-- rev_type
CREATE OR REPLACE TABLE `bounce-survivor---puzzle-rpg.dashboard_table.rev_type`
PARTITION BY event_date
...
-- compare_version 
CREATE OR REPLACE TABLE `bounce-survivor---puzzle-rpg.dashboard_table.compare_version`
PARTITION BY event_date
CLUSTER BY  country,version, platform AS
WITH sessions AS (
  SELECT
...
-- iap_sequence
CREATE OR REPLACE TABLE `bounce-survivor---puzzle-rpg.dashboard_table.iap_sequence`
PARTITION BY event_date
CLUSTER BY  country,version, platform AS
...
-- iap_monet
CREATE OR REPLACE TABLE `bounce-survivor---puzzle-rpg.dashboard_table.iap_monet`
PARTITION BY event_date
CLUSTER BY  country,version, platform AS
WITH sessions AS (
  SELECT
  ...
  SELECT 
    ...
    count(distinct user_pseudo_id) as pay_user, 
    count(distinct case when Day_since_fo <= 7 then user_pseudo_id else null end) as pay_new_user,
    sum(event_value_in_usd) as revenue,
    sum(case when Day_since_fo <= 7 then event_value_in_usd else 0 end) as revenue_new_user 
  FROM `bounce-survivor---puzzle-rpg.flatten_table.iap_chapter`
  group by event_date, country, platform, version
)
select 
  a.event_date, a.country, a.version, a.platform, a.active_user, b.transactions, b.pay_user, b.pay_new_user, b.revenue,b.revenue_new_user
from sessions a left join iap b 
on a.event_date = b.event_date and a.country = b.country and a.platform = b.platform and a.version = b.version; 
...
-- chapter_play
CREATE OR REPLACE TABLE `bounce-survivor---puzzle-rpg.dashboard_table.chapter_play`
PARTITION BY event_date
...
WITH start_chapter AS (
  ...
  WHERE CAST(SPLIT(chapter_wave, "_")[OFFSET(1)] AS INT64) = 1   -- chỉ lấy wave = 1
  GROUP BY 1,2,3,4
),
...
-- chapter_play_mode
CREATE OR REPLACE TABLE `bounce-survivor---puzzle-rpg.dashboard_table.chapter_play_mode`
PARTITION BY event_date
CLUSTER BY   version, platform AS
WITH start_chapter AS (
  SELECT 
    event_date, platform, version,
...
-- hero_winrate
CREATE OR REPLACE TABLE `bounce-survivor---puzzle-rpg.dashboard_table.hero_winrate`
PARTITION BY event_date
CLUSTER BY version, platform, chapter, hero AS
...
win_general AS (
  SELECT 
    ...
    CAST(SPLIT(chapter_wave, "_")[OFFSET(0)] AS INT64) AS chapter,
    'general' AS hero,
    IFNULL(COUNT(user_pseudo_id), 0) AS win_event,
    IFNULL(COUNT(DISTINCT user_pseudo_id), 0) AS win_user
  FROM `bounce-survivor---puzzle-rpg.flatten_table.win_chapter`
  GROUP BY 1,2,3,4,5
),
hero_start AS (
...
-- daily_challenge 
CREATE OR REPLACE TABLE `bounce-survivor---puzzle-rpg.dashboard_table.daily_challenge`
PARTITION BY event_date
CLUSTER BY version, platform  AS
with b as( 
select event_date, version, platform, 
...
from `bounce-survivor---puzzle-rpg.flatten_table.session_start`
where user_pseudo_id in (select distinct user_pseudo_id from `bounce-survivor---puzzle-rpg.flatten_table.unlock_scavenge_beyond_feature`)
group by 1,2,3
)
select a.event_date, a.version, a.platform, a.user_start_chapter, b.user_start_daily_challenge
from a left join b using(event_date, version, platform) ;
...
-- day_sesion_play
CREATE OR REPLACE TABLE `bounce-survivor---puzzle-rpg.dashboard_table.day_sesion_play`
PARTITION BY event_date
CLUSTER BY version, platform, type  AS
with first_open_dates AS (
  ...
  select a.user_pseudo_id, a.event_date, a.platform, a.version, a.ga_session_number,
  ...
  select event_date, platform, version, st.Day_since_fo, 
  ...
  select event_date, platform, version, Day_since_fo, 
  ...
  select event_date, platform, version, ga_session_number, 
  count(distinct user_pseudo_id) as num_user_play,
  sum(num_event_play) as num_play,
  sum(num_chapter_play) as num_chapter_play
  from st group by 1,2,3,4
...
select a.event_date, a.platform, a.version, a.Day_since_fo as day_session, 
  a.num_play, a.num_user_play, a.num_chapter_play, b.num_win, b.num_user_win, b.num_chapter_win,
  'cohort_day' as type 
from cohort_day_start a
left join cohort_day_win b using (event_date, platform, version, Day_since_fo)
union all
select a.event_date, a.platform, a.version, a.ga_session_number as day_session, 
  a.num_play, a.num_user_play, a.num_chapter_play, b.num_win, b.num_user_win, b.num_chapter_win,
  'session_number' as type 
from session_number_start a 
left join session_number_win b using (event_date, platform, version, ga_session_number)
